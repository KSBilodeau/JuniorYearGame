...
void KMap::loadMap()
{
    for (int y = -1; y < 1; y++)
    {
        for (int x = -1; x < 1; x++)
        {
            ...
            block.loadDefaultTile(std::make_pair(0, 1));
...
void KMap::attachTileCollisionEvent(int &x, int &y, int &id, int &tileSheet, int &storageID, std::pair<int, int> tileID, EventType eventType)
{
    int blockX = x - (x % (TILE_WIDTH * BLOCK_WIDTH));
    int blockY = y - (y % (TILE_HEIGHT * BLOCK_HEIGHT));
    ...
    mBlockList.at(std::make_pair(blockX, blockY)).attachTileCollsionEvent(tileX, tileY, id, tileSheet, storageID, tileID, eventType);
}
...
void KMap::addObject(GameObject *object)
{
    objects.push_back(object);
}

void KMap::removeObject(GameObject *object)
{
    objects.erase(std::remove(objects.begin(), objects.end(), object), objects.end());
}

void KMap::addObjects(int argc, ...)
{
    std::va_list args;
    va_start(args, argc);
    
    for (int i = 0; i < argc; i++)
        objects.push_back(va_arg(args, GameObject*));
...
bool KMap::checkCollision(std::pair<int, int> blockCoords, SDL_Rect objectRect, bool isPlayer)
{
    // Collision flag
        ...
        for (int x = blockCoords.first - 2; x < blockCoords.first + 2; x++)
        {
            if (mBlockList.count(std::make_pair(x, y)) != 0)
                for (std::pair<std::pair<int, int>, KTile> mappedTile : mBlockList.at(std::make_pair(x, y)).getMappedTiles())
                    colliding = colliding | mappedTile.second.checkCollision(objectRect, mappedTile.second.getRectangle(), isPlayer);
            else
            {
                loadBlock(std::make_pair(x, y));
            }
...
bool KMap::checkCollision(GameObject *object)
{
    // Collision flag
    bool colliding = false;
    ...
    {
        if (object2->getType() == GameObjectType::player)
        {
            colliding = colliding | ((Player*) object2)->checkCollision(object);
        }
        else if (object2->getType() == GameObjectType::boomerang)
        {
            colliding = colliding | ((KBoomerang*) object2)->checkCollision(object);
        }
    }
