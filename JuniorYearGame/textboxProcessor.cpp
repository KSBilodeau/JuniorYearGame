...
bool TextboxEventProcessor::loadTextboxSequenceFromFile(std::string path)
{
    ...
    while (!input.eof())
    {
        ...
        if (line.find('\"') != std::string::npos)
        {
            ...
            int id = textureRegistry.getRegistrySize();
            
            KTexture temp;
            temp.loadFromString(line, {0, 0, 0});
            textureRegistry.registerTexture(id, temp);
            
            Textbox textbox;
            textbox.createTextbox(textureRegistry.requestAccess(6), textureRegistry.requestAccess(id), (SCREEN_WIDTH / 2) - (textureRegistry.requestAccess(6)->getWidth() * 1.776 / 2), (SCREEN_HEIGHT / 1.20) - (textureRegistry.requestAccess(6)->getHeight() * 1.776 / 2));
