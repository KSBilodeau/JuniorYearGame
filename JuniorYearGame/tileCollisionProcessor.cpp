    ...
    while (!input.eof())
    {
        ...
        else if (line.find("EVENT_TYPE") != std::string::npos)
        {
            if (line.find("TEXTBOX_SEQUENCE") != std::string::npos)
            {
                ...
                registerEvent(temp, tileX, tileY, selectedTileSheet, tileID);
            }
        }
        else if (line.find("TILE_ID") != std::string::npos)
        {
            std::istringstream iss(line);
            std::vector<std::string> id;
            ...
            tileID.first = std::stoi(id[1]);
            tileID.second = std::stoi(id[2]);
        }
        else if (line.find("TILE_SHEET") != std::string::npos)
        {
            std::istringstream iss(line);
            std::vector<std::string> tileSheet;
            
            std::copy(std::istream_iterator<std::string>(iss), std::istream_iterator<std::string>(), std::back_inserter(tileSheet));
            
            selectedTileSheet = std::stoi(tileSheet[1]);
        }
...
void TileCollisionProcessor::registerEvent(std::shared_ptr<TileCollisionEvent> &tileCollisionEvent, int &tileX, int &tileY, int &tileSheet, std::pair<int, int> tileID)
{
    mEvents.insert(std::make_pair(mEvents.size(), tileCollisionEvent));
    
    eventHandler.registerEvent(tileCollisionEvent);
    
    gMap.attachTileCollisionEvent(tileX, tileY, tileCollisionEvent->attachedEventID, tileSheet, tileCollisionEvent->eventID, tileID, tileCollisionEvent->attachedEventType);
}
